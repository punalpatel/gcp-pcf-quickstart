...
func NewSdk(target string, creds config.OpsManagerCredentials, logger log.Logger) (*Sdk, error) {
	client, err := network.NewOAuthClient(target, creds.Username, creds.Password, "", "", creds.SkipSSLVerification, true, time.Duration(requestTimeout)*time.Second, time.Duration(requestTimeout)*time.Second)
	if err != nil {
		return nil, err
	}
	...
	unuthedClient := network.NewUnauthenticatedClient(target, creds.SkipSSLVerification, time.Duration(requestTimeout)*time.Second, time.Duration(requestTimeout)*time.Second)
	...
	return &Sdk{target: target,
		...
		api: api.New(api.ApiInput{
			...
			UnauthedClient:         unuthedClient,
			UnauthedProgressClient: unuthedClient,
			Logger:                 &logger,
		}),
...
func (om *Sdk) SetupBosh(

	// UpdateStagedDirectorProperties
	director commands.DirectorConfiguration,
	iaas commands.GCPIaaSConfiguration,

	// UpdateStagedDirectorAvailabilityZones
	azs []*api.AZ,

	// UpdateStagedDirectorNetworks
	networks api.NetworksConfigurationOutput,

	// UpdateStagedDirectorNetworkAndAZ
	networkAssignment tilestructs.Network,

	resources commands.ResourceConfiguration) error {
	cmd := commands.NewConfigureDirector(func() []string {
		return []string{}
	}, om.api, om.logger)
	...
	azBytes, err := yaml.Marshal(azs)
	if err != nil {
		return err
	}

	networksBytes, err := yaml.Marshal(networks)
	if err != nil {
		return err
	}
...
func (om *Sdk) ConfigureProduct(name, networks, properties string, resources string) error {
	cmd := commands.NewConfigureProduct(func() []string {
		return []string{}
	}, om.api, om.logger)
...
func (om *Sdk) curl(path, method string, data io.Reader) ([]byte, error) {
	...
	req := api.RequestServiceCurlInput{
		Path:   path,
		Method: method,
		Data:   data,
	}

	resp, err := om.api.Curl(req)
